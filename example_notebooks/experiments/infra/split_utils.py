...
def resolve_normalized_splits(config) -> dict[str, dict[str, str]]:
    ...
    for normalized_role in ("train", "val", "test_holdout"):
        campaigns_path, stats_path = _resolve_split_paths(config, normalized_role)
...
def _resolve_split_paths(config, normalized_role: str) -> tuple[str, str]:
    ...
    for alias in _NORMALIZED_SPLIT_ALIASES[normalized_role]:
        split_cfg = config.data_config.get(alias)
    ...
    available = sorted(config.data_config.keys())
    raise ValueError(
